...
function ep(cover, title, date, epNum, audioSource, description) {
  return `
...
for (const key in EpisodesData) {
  const [cover, title, date, epNum, audioSource, description] = [
  ...
  Main.innerHTML += ep(cover, title, date, epNum, audioSource, description);
}
