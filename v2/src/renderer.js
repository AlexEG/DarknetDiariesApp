...
function ep(cover, title, date, duration, epNum, audioSource, description) {
  return `
    ...
    >
      ...
      <button class="play-Pause-btn opacity-0 group-hover:opacity-100 absolute bottom-1.5 right-1.5 transition-opacity">
      <svg
        ...
        stroke-width="1.5"
        stroke="#f00"
        class="w-9 h-9 bg-black/30 rounded-full hover:drop-shadow-[0_0_30px_#000] transition delay-75 duration-500"
        data-playing="false"
      >
        <path
    ...
    <button
          ...
          >
            <svg
              ...
              stroke-width="1.0"
              stroke="#f00"
              class="w-6 h-6 rounded-full hover:drop-shadow-[0_0_30px_#000] transition delay-75 duration-500"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
...
function playPauseBtnClickHandler() {
  ...
  const [headerAudioSource] = [document.querySelector("#header-audio-source")];
  ...
  playPauseBtn.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      // --- Btn Functionality --- //
      const [title, date, duration, audioSource, description] = [
